...
#
# bash <(curl -s https://raw.githubusercontent.com/vieko/dotfiles/master/bootstrap.sh)
...
if [[ ! -d ~/.dotfiles ]]; then
  ...
  if [[ $USER == vieko ]]; then
    dfrepo=git@github.com:vieko/dotfiles.git
  else
    dfrepo=https://github.com/vieko/dotfiles
  fi
